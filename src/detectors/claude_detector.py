...
class ClaudeDetectorWithPresidio(ClaudeDetector):
    """Claude-based PII detector that uses Presidio entity output as context."""
    
    def detect(self, email: Email) -> DetectionResult:
        """
        Detect PII in email using Claude with Presidio entities as context.
        ...
        if self.use_few_shot:
            ...
            prompt = build_5_shot_prompt_with_presidio(email, no_pii_examples, with_pii_examples)
        else:
            prompt = build_zero_shot_prompt_with_presidio(email)
...
def create_haiku_detectors_with_presidio(labeled_dataset: List[Email]) -> List[ClaudeDetectorWithPresidio]:
    """
    Create Haiku detector instances with Presidio support (zero-shot and 5-shot).
    
    Args:
        labeled_dataset: Labeled emails for 5-shot learning (should have presidio_entities populated)
        
    Returns:
        List of [haiku_zeroshot_with_presidio, haiku_5shot_with_presidio]
    """
    return [
        ClaudeDetectorWithPresidio(
            name="haiku_zeroshot_with_presidio",
            model=HAIKU_MODEL,
            use_few_shot=False
        ),
        ClaudeDetectorWithPresidio(
            name="haiku_5shot_with_presidio",
            model=HAIKU_MODEL,
            use_few_shot=True,
            labeled_dataset=labeled_dataset
        )
...
def create_sonnet_detectors_with_presidio(labeled_dataset: List[Email]) -> List[ClaudeDetectorWithPresidio]:
    """
    Create Sonnet detector instances with Presidio support (zero-shot and 5-shot).
    
    Args:
        labeled_dataset: Labeled emails for 5-shot learning (should have presidio_entities populated)
        
    Returns:
        List of [sonnet_zeroshot_with_presidio, sonnet_5shot_with_presidio]
    """
    return [
        ClaudeDetectorWithPresidio(
            name="sonnet_zeroshot_with_presidio",
            model=SONNET_MODEL,
            use_few_shot=False
        ),
        ClaudeDetectorWithPresidio(
            name="sonnet_5shot_with_presidio",
            model=SONNET_MODEL,
            use_few_shot=True,
            labeled_dataset=labeled_dataset
        )
