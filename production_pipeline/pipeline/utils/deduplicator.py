...
class EmailDeduplicator:
    ...
    def get_duplicate_id_mapping(self, original_emails: List[Email]) -> Dict[int, Tuple[bool, int]]:
        """
        Create a mapping from original email IDs to (is_duplicate, representative_id).
        
        Args:
            original_emails: All original emails (before deduplication)
            
        Returns:
            Dictionary mapping original_id -> (is_duplicate, representative_id)
        """
        # Perform deduplication to get groups
        unique_emails, _ = self.deduplicate(original_emails)
        
        # Build mapping
        id_mapping = {}
        
        # Build hash -> representative ID mapping
        hash_to_rep_id = {}
        for email in unique_emails:
            ...
            if self.is_forwarded(email.message_body):
                ...
                msg_hash = self.hash_message(email.message_body)
            hash_to_rep_id[msg_hash] = email.id
        
        # Map all original emails
        for email in original_emails:
            ...
            representative_id = hash_to_rep_id[msg_hash]
            is_duplicate = (email.id != representative_id)
            
            id_mapping[email.id] = (is_duplicate, representative_id)
